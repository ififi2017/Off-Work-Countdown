...
- iOS runs every shared rule in Swift since plan 019 R1–R3, in
  ...
  in Swift alone.
- `src-mobile/ios/App/App/Resources/CountdownRules.js` is still generated by
  `npm run build:ios-native-rules` and shipped, but since plan 019 R3 it carries
  no entry points and nothing evaluates it. Plan 019 R4 deletes it together with
  its generation, packaging and checks; until then it keeps building.
...
**Generate the rules bundle first.** It is not committed, and the project still
lists `CountdownRules.js` as a resource until plan 019 R4 removes it, so a fresh
clone generates it before building:

```bash
npm run build:ios-native-rules
```

It holds no rules any more (plan 019 R4 removes it). A change to shift,
reminder, summary or salary rules needs the matching change in
`ScheduleRules.swift`, `ReminderRules.swift` or `SummaryRules.swift` and
