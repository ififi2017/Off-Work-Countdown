...
/// The grid arithmetic behind the chain, kept in a pure model so both
...
extension FocusStore {
  ...
  func focusChain(for session: FocusSession, at date: Date = .now) -> [FocusChainLeg] {
      ...
      nextTask.completedAt == nil
    {
      ...
      let done =
        completedFocusBlocks(for: nextTask)
  ...
  func addableFocusBlocks(at date: Date = .now) -> [FocusDayCanvasModel.Block] {
    ...
    var after: Date?
    var result: [FocusDayCanvasModel.Block] = []
    while case .success(let target) = focusContinuationTarget(
      taskID: taskID, at: date, after: after),
  ...
  @discardableResult
  func addFocusPomodoroToRunningTask(
    count: Int = 1,
    matching request: FocusActivityRequest? = nil,
    at date: Date = .now
  ) -> RecordCommand<Bool> {
    records.submitCommand { [self] in
      guard request.map({ matchesFocusActivity($0, at: date) }) ?? true,
        count > 0, count <= addableFocusBlocks(at: date).count,
        let taskID = lockScreenContinuationTaskID(at: date)
      else { return false }
      for _ in 0..<count {
        guard case .success = addOneFocusBlock(taskID: taskID, at: date).synchronousResult else {
          return false
        }
      }
      return true
