...
final class OffWorkStore {
    /// The one persisted store this process uses.
    ///
    /// A Live Activity button performs its intent inside the app process,
    /// which iOS may have launched in the background with no scene and no
    /// `RootView` at all — so the store cannot be something a view owns. Both
    /// entry points resolve to this instead, which is also what keeps a
    /// Lock Screen tap and the foreground UI from writing the same archive
    /// through two separate copies of it.
    static let shared = OffWorkStore(records: .persisted())
    ...
    func shiftReminders(at date: Date = .now) throws -> [NativeReminder] {
        ...
        return applyingFocusBreakTakeover(to: (includeCurrent ? current : []) + next, at: date)
    }
    ...
    /// `OffWorkStore+FocusTemplates` because `focusPlanning` is
    /// `private(set)` on purpose — the editor owns what a usual day looks
    /// like, not how planning state is stored.
    func appendFocusTemplate(_ template: FocusTemplate) {
        focusPlanning.templates.append(template)
        persistFocusPlanning()
    }
    ...
    /// Not private: the template editor in `OffWorkStore+FocusTemplates`
    ...
    func addScheduledFocusTask(
        ...
        if plus.isAuthorized {
            ...
            openPaidOrRun(
                .focus,
                action: .addFocusTaskInNextBlock(
                    title: trimmed,
                    pomodoros: 1,
                    icon: icon,
                    isFavorite: isFavorite
    ...
    func performPendingPlusAction(_ action: PlusPendingAction) {
        ...
        case .addFocusTaskInNextBlock(let title, let pomodoros, let icon, let isFavorite):
            _ = createFocusTaskInNextEmptyBlock(
                title: title,
                pomodoros: pomodoros,
                icon: icon,
                isFavorite: isFavorite
    ...
    /// Not private: the placement helpers in `OffWorkStore+FocusPlacement`
    ...
    private func focusBlockEndAlert(for session: FocusSession) -> NotificationService.FocusAlert {
        ...
        if let task {
            ...
            parts.append(t("focusActivityPomodoro", values: [
        ...
        if completesFocusDay(after: session, at: session.startedAt) {
            ...
            if let end = plannedFocusBreakEnd(kind: kind, at: session.plannedEndAt) {
                ...
                parts.append(t("focusBreakUntil", values: [
    ...
    private func focusBreakEndAlert(endingAt end: Date) -> NotificationService.FocusAlert {
        ...
        if let next, let title = next.taskTitle {
            body = t("focusActivityNextUp", values: [
                "task": title,
