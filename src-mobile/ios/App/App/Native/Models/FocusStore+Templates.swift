...
extension FocusStore {
  ...
  @discardableResult
  func saveFocusTemplate(name: String, slots: [FocusTemplateSlot]) -> RecordCommand<FocusTemplate?>
  {
    records.submitCommand { [self] in
      let trimmed = name.trimmingCharacters(in: .whitespacesAndNewlines)
      guard plus.isAuthorized, !trimmed.isEmpty, !FocusTemplate.tasks(from: slots).isEmpty else {
        return nil
      }
      let now = Date.now
      ...
      appendFocusTemplate(template).synchronousResult
      return template
    }
  }

  @discardableResult
  func updateFocusTemplate(
    _ template: FocusTemplate, name: String, slots: [FocusTemplateSlot], at date: Date = .now
  ) -> RecordCommand<Bool> {
  ...
  func focusTemplateFit(_ template: FocusTemplate, at date: Date = .now) -> (
    fits: Int, dropped: Int
  ) {
