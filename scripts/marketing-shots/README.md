...
**标题只能在网页端选。** 提交 API 接受带 `title` 的 PUT 并返回 200，读回来却仍是主名称。
每个语言用哪个保留名，要在合作伙伴中心那个提交的商品页里，逐个语言在「产品名称」下拉框
里选（对照表在 `listing-copy.mjs` 的 `title`）。网页端动过的提交 API 往往就写不进去了，
所以顺序是：`msstore:sync` → 网页端选标题 → 网页端提交。
