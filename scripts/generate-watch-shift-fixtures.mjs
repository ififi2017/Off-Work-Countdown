...
function fixture(scenario) {
  ...
  const initial = JSON.parse(context.OWCNative.snapshot(JSON.stringify(baseInput)));
  if (scenario.overtimeHours) {
    baseInput.overtimeEndAtMs = initial.plannedEndAtMs + scenario.overtimeHours * hour;
  }
  const result = JSON.parse(context.OWCNative.snapshot(JSON.stringify(baseInput)));
  const finishedAtMs = scenario.finishedAt ? Date.parse(scenario.finishedAt) : null;
  const frozen = finishedAtMs == null ? result : JSON.parse(context.OWCNative.snapshot(JSON.stringify({
    ...baseInput,
    nowMs: finishedAtMs,
  })));
