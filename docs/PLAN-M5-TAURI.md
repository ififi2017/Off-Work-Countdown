...
**留给 P2 的观感问题**：透明标题栏下内容从 y=0 开始，红绿灯按钮紧贴着卡片标题，macOS 上需要给顶部留出约 28px 内边距。
...
- 托盘菜单文案的本地化。托盘标题按设计是语言无关的时间格式，但菜单项需要 Rust 侧知道用户选的语言。P1 暂用英文，随 P2 的托盘标题一并处理
...
- ~~主窗口被拉宽时观感失衡~~ → **在 Tauri 侧设 `maxWidth: 560`**，不改 CSS。P1 已落地
