...
- 合入 #205 的历史补填实现，保留以完整 `ScheduleSnapshot` 为入口的记录解析链路，统一使用共享源码中的排班覆盖机制。
- 没有工时快照的旧补填继续沿用旧版的生效边界；本轮新增的冻结历史补填可用于职业区间内的固定排班历史，包括切回固定模式后的历史。两者不以一个全局开关互相覆盖。
- 切换自由排班时保留 #205 的规则生效日期下限，避免把规则生效前的历史自动写成逐日排班。
- 新月历取代旧月历及其说明文案，19 语言检查通过；原 UI 走查证据仍适用于布局。本次整合重点回归数据与规则。
- 整合后的完整串行 iOS 回归 791 项／50 组、Watch 22 项／3 组全部通过；JavaScript 单元测试 411 项、lint、工程结构与 19 语言检查通过。日志为 `owc-pr207-merge-ios-final.log`、`owc-pr207-merge-watch.log`、`owc-pr207-merge-js.log`。
- 首轮 iOS 仅新增兼容用例失败：测试重复选择原班次，被正确视为无改动。改为实际更换夜班，并断言冻结工时与 20:00 班次后完整重跑通过，未改变产品的无改动处理逻辑。
