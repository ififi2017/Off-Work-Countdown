...
20. **窗口形态** — 🟡 实现完成、Windows 实机验收待办。主窗口默认 430×430、宽高均仅允许在 420–450 内调整并禁止最大化；桌面标题单行、时间双栏、底部操作固定，分享页完整覆盖 WebView。迷你窗固定 252×104，使用无白边的满铺深色面板；主窗与迷你窗均补齐真实拖动权限（详见 M5 计划决策 7）
