...
18. **菜单栏/托盘实时倒计时** —— **桌面端的核心卖点**。macOS 菜单栏直接显示 `2:13:45`；Windows 改用置顶迷你窗。秒级刷新走 Rust 侧计时循环，与 webview 是否可见解耦
19. **原生能力** —— 系统通知、开机自启、全局快捷键（唤起/隐藏）
20. **窗口形态** —— 默认小尺寸常驻窗 + 置顶迷你窗（详见 M5 计划决策 7）
21. **自动更新** —— `tauri-plugin-updater`，更新源托管在 GitHub Releases
22. ~~**签名与公证**~~ —— 不做，改为在 README 写清安装步骤
