...
| T13 | IN_PROGRESS | 拆为两部分。**T13a 已完成**：`focus/FocusPlanner`（网格、可开始窗口、边界、结束原因、溢出）、`FocusSessionIdentity`（SHA-256 派生，与交接包两个向量一致）、`FocusEngine`（开始条件与互斥、计划块内开始、自然结束按计划终点结算、任务达预估才完成、自动休息与派生 id、跳过、重启/同步后收敛为一个进行中会话、冷启动从历史恢复下一步）。移植 `FocusPlannerTests` 的 13 条纯函数用例，另有 18 条生命周期测试对应 `FocusStoreTests` 的同名行为；4 项植入错误均被捕获。**T13b 待做**：计划分配、模板、放置、收藏、画布与由计划驱动的自动会话（`restore/refreshScheduledFocus`）、`FocusLiveChain`。 |
| T14–T20 | NOT_STARTED | — |
