...
| QA-041 | JVM | NOT_RUN | Partial | `SummaryFixtureTest.salaryInputKeepsEmptyDistinctFromZeroAndRejectsInvalidNumbers`; `RecordsTransferTest.invalidSalaryTextInARecoveredArchiveDoesNotBecomeAnAmount` | Frozen iOS/Kotlin preserve malformed salary text in preferences while calculations return no amount; clarify catalog rejection wording and finish invalid editor-input checks without inventing an Android-only archive rule. |
