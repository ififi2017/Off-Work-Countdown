...
2026-09-10 记录页交互修正：恢复周／月／年摘要内的时间分配占比条；工时和收入值按整行（含辅助文字）垂直居中。占比条首次显示默认选择工作，无工作时选择第一个非空分类；重复点击当前分类不清空说明。人生摘要移除“按当前设置……”的重复工作时长句子。日期详情的 24 小时条改为条上方固定说明，默认选择第一段工作，点击其他时段直接切换，不再弹出玻璃 popover；选中内容使用当前模型解析，数据刷新不会留下旧时长。年展开月行布局本次不改，待用户确认整改建议。
