...
2026-09-07 专注实时活动的 payload 带上「运行中的一段 + 它换来的休息 +
计划里排在后面的那一块」的绝对时间与文案。扩展获得重绘机会时按墙钟选段；预告块只显示
开始时间，不自动开始专注。App 恢复执行时，只在休息窗口尚未结束的情况下补出休息 session。
自然到期按计划结束时间收口专注记录，实际返回时间只用于判断休息窗口，避免两条记录重叠。

通知预先安排本段结束和休息结束两条。锁屏「再加一个番茄」通过 `LiveActivityIntent` 修改计划，
等待当前专注通知重排后再更新实时活动；通知串行写入，旧请求不能清掉同一 session 的新版，
过期提醒不补发。按钮只有在当前专注仍运行时才显示；Intent 执行前也结算过期 session。
`OffWorkStore.shared` 与 `LiveActivityService.shared` 是 App 和后台 Intent 共用的进程内实例。

**离线能力边界仍未关闭：** `TimelineView` 的边界只是重绘请求，不是 ActivityKit 的定时更新。
App 挂起时，链尾 `Task.sleep` 也不能保证准时结束活动。当前采用到零停止的系统区间计时，
`staleDate` 设在本段结束时，避免旧阶段被标为新鲜直到链尾；恢复执行后重新选段或回收活动。
不能将「payload 已含整条链」等同于「锁屏准时自动换段与退出」。这两项仍需受支持的更新机制
及真机验收；本次没有增加服务器、推送身份或后台保活。参考
