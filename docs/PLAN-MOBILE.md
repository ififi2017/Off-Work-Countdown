...
2026-09-07 专注实时活动改为链式：payload 里带上「运行中的一段 + 它换来的休息 +
计划里排在后面的那一块」的绝对时间与成品文案，Widget 扩展按墙钟自己走到当前一段，不再
停在「本段已完成」。手机在番茄里几乎全程休眠，所以整条链必须提前写好；App 拿到执行时间后
把休息落成真正的 session（只在休息窗口尚未结束时补，晚回来不追写），计划里没有下一块时
在链尾收回实时活动。通知同样一次写两条：结束通知带任务名、第几个番茄和休息到几点，休息
结束通知带下一块任务和开始时间。实时活动新增「再加一个番茄」按钮（`LiveActivityIntent`，
与 Widget 共享 `Models/FocusActivityIntents.swift`），本班次剩余格子都被别的任务占满时置灰。
`OffWorkStore.shared` 与 `LiveActivityService.shared` 因此成为进程内唯一实例——按钮会在没有
场景的后台进程里执行 intent，store 不能再由视图持有。

已知边界：卡片自己换段依赖系统给扩展一次绘制机会。`TimelineView` 只要来一段的边界时刻
（不再每秒），这是能提出的最小请求，但模拟器在锁屏状态下整段时间都没有重绘扩展——真机待
验。内容本身不会错：payload 里已经带着整条链，任何一次重绘（App 重新发布、展开灵动岛、
系统自己刷新）都会显示当下那一段。锁屏期间真正送达用户的是两条通知，这条链路已在模拟器
上跑通。
