...
- iPhone 竖屏使用系统 TabView；iPhone 横屏使用紧凑侧栏；iPad 使用可收起侧边栏和独立顶部导航，每个入口保留独立
...
2026-09-06 玻璃交互补充：保留用户指定的独立运动方向：侧边栏从左侧进入／退出，顶部导航从上方进入／退出。使用 `NavigationSplitView(.balanced)` 管理侧边栏，顶部导航作为独立 overlay，以 `OWCMotion.navigation` 驱动 `.move(edge: .top)`；减少动态效果时改用淡入淡出。不能为了玻璃效果改成系统 `.sidebarAdaptable` 的整栏变形。顶部内部使用系统分段选择控件提供交互玻璃，侧边栏按钮独立于三个选项，三个入口保留各自导航路径。分段控件与 Tab 栏的具体透镜表现需实机比对，不以静态截图证明一致。参考 [Apple 的 Liquid Glass 教程](https://developer.apple.com/videos/play/wwdc2025/323/)。
