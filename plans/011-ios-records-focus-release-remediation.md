...
## 十、真机 / TestFlight 验收进展（2026-09-04）

用户在 TestFlight 上用 iPhone 与 iPad 两台真机跑了第一轮，结论分开记：

### 已通过

- 同一 iCloud 账户下，记录在两台设备之间无缝传播。
- 番茄钟的计划、模板与计时设置一并传播（它们在 `focusPlanningConfiguration` 这条同步实体里）。

### 第八节仍未验证

上一条只覆盖了「正常传播」。以下都还没做，不能因为传播通了就一起签收：

- 冲突合并回写：两台设备离线各改同一天，再同时上线。
- 双 open session 收敛：两台设备同时在跑番茄钟。
- 删除 fence：「删除全部」后另一台设备的低世代 zone 是否被清掉。
- 账户切换、离线与 iCloud 额度不足。
- Production CloudKit schema、record type、索引与权限核对。
- 后台通知声音、锁屏、灵动岛、跨午夜恢复、通知权限拒绝后的恢复。
- 工作倒计时与专注 / 休息 Live Activity 的单实例优先级切换。
- iPhone / iPad 的深浅色、RTL、超大字体、降低动态效果、降低透明度、无颜色区分、旋转、分屏。

### 新增结论：番茄钟需要一次 UI 重做

同一轮真机试用里，用户判断番茄钟「功能有点乱」：快捷创建任务、一天的排布和常用天数这几处的信息架构不符合使用习惯，需要重排，不是修补。

这不推翻本计划——011 修的是闭环、跨夜恢复、模板幂等、双设备收敛和通知这些**行为**阻断，那些修复继续有效。要重做的是**排布**。因此：

- 本计划的番茄钟部分维持现状，继续按第八节验收行为；
- 重排单独立项，不要在 011 里顺手改结构，否则真机验收的对象会在验收过程中变形。
