...
| 数据 | 默认位置/动作 | 发布前需要确认 |
|---|---|---|
| 工资、班次、职业、任务与记录 | 本地Room；随 Android 系统备份/设备转移 | 系统备份由 Google 处理（Android 9+ 设锁屏时端到端加密）；如实填写 Data safety；日志和组件无内容 |
...
备份策略验证 cloud backup 与 device-to-device transfer：业务库可恢复（QA-138～QA-140），密钥/Token/购买缓存不进入备份；隐私文案与 Auto Backup 实际行为一致。[A16], [A28]
