...
**目标：以 Kotlin + Jetpack Compose + 稳定版 Material3（DoneAt 设计 token 实现 Expressive 风格）原生实现 iOS 功能，并完成 Google Play 发布准备。**
...
| 2 · 同步 | D-02（1.2 修订） | 首发以 Android 系统备份/设备转移换机恢复（业务库纳入备份，购买缓存/密钥/令牌排除）；Drive appDataFolder 多设备同步延至首发后（T22），届时默认关闭、主动授权 | 真机换机恢复验收；后续阶段的 OAuth 配置与双设备验收 |
...
| 5 · 验证服务 | D-08（1.2 修订） | 首发纯客户端 Play Billing（查询 + 公钥签名校验 + 客户端 acknowledge），与 iOS 纯客户端 StoreKit 对齐；服务端验证延至首发后（T21），届时优先放入现有 Vercel | 首发只需许可公钥；后续阶段的部署与凭据 |
| 6 · Play 账户 | D-11 | 按实际 Play Console 账户确定验证、测试与生产访问要求 | 是否已有账户、个人/组织、创建时间、生产访问状态均未知 |
| 1.2 新增 | D-12 | Release 只用稳定 Compose/Material3；Expressive 风格由 designsystem token 实现 | 官方 Expressive 稳定后的切换评估 |
...
**1.2 修订（2026-09-23）**：负责人评审后回复“其他的可以按照你的建议更新好”，据此修订 D-02、D-08，新增 D-12，并把任务进度收敛到 `docs/android/progress.md`（`tasks.json` 只定义依赖与范围）。仍是文档决策更新：未部署云服务、未操作 Play Console。
...
| `tasks.json` | 28 张任务卡的依赖与范围（不含状态） | 选择任务；进度只在 `docs/android/progress.md` |
| `test-catalog.json` | 140 条测试规范，初始全部 NOT_RUN | 自动建立测试追踪表 |
| `references.json` | 52 项源码和官方参考网址 | 按引用 ID 定位资料 |
| `decisions.json` | 六项确认映射、其余 D 项与外部配置状态 | 开工、交接及发布前必读 |
| `CHANGELOG.md` | 1.1、1.2 的变更和未执行范围 | 确认没有误用旧版结论 |
...
- **已确认决策**：D-01 收费模式、D-02 首发换机恢复/Drive 延后、D-03 首发互通边界、D-05 手表延后、D-08 首发客户端验证/服务端延后、D-12 稳定 Material3；D-11 仅确认按实际账户核验的流程。具体状态见 05 与 decisions.json。
...
“完整移植”必须覆盖：排班与倒计时、收入、记录及人生、专注、提醒、小组件、数据迁移、隐私、付费体系、19 语言、手机/平板适配，以及已确认的首发换机恢复方案（Drive 同步为后续阶段）。分阶段开发只是实施顺序，不代表可以把未完成部分叫作最终完整版。

已确认手机/平板首发覆盖 FR-01～FR-20：FR-15 首发为系统备份/设备转移换机恢复，FR-16 首发为客户端 Play 验证。Drive 同步（T22、QA-110～QA-122）与服务端验证（T21）属首发后阶段，不阻塞首发，也不能标为已完成。Apple Watch 是独立 watchOS 目标；Wear OS 的 FR-21/T27/QA-137 保留为后续阶段，当前不计入首发发布阻塞，也不能标为已完成。iOS/Android 首发仅做 JSON 数据迁移，购买权益独立，不做自动跨平台同步，不新增 DoneAt 账号。购买验证服务获方案批准，不表示生产部署和发布已执行或可以跳过操作授权。
...
所有范围内 FR 条目能追溯到实现和测试；源码审计无未解释缺项；TS→Kotlin、Swift 特有规则→Kotlin 差分通过；旧备份及双向往返通过；Release 构建通过；未用假的支付或备份恢复充当生产实现；真机完成权限、后台、恢复购买、换机备份恢复和离线场景；19 语言与大屏验收；负责人完成真实 Play 配置并签收。
