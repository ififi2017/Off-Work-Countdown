...
### 2026-09-05 用户确认的排班与点阵补充

本节优先于下文早期的「只按当日观察计入」口径：用户保存并生效的排班是持续有效的默认安排。
已过去的日期按当时保存的排班计入记录画布与汇总，无需每天打开 App、开始或停止计时；
手动覆盖、请假、补班与登记加班仍沿用原有优先级。当天尚未发生的部分和未来日期继续标为计划／推算。
首次有效排班之前由人生档案补出的历史仍是推算，不倒填持久化观察或逐日记录。
原始事件列表继续展示用户写入的观察、修正和例外；画布与统计同时读取有效排班。

年与人生继续使用响应式比例采样，不固定一格的时间单位，不改成年历。人生的工作阶段在
展示层以现在分为「已走过」和「未来推算」两个独立选择区间，仍共用工作分类色，未来部分
增加斜纹。分段不改变人生档案、退休边界或工时算法。阶段区间只表达人生档案中的时间，
工时按生效排班与记录计算。
