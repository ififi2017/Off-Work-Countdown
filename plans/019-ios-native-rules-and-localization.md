...
- **状态**：TODO — 2026-09-13 用户决定方向并确认本地化方案；尚未开工，`AGENTS.md` 尚未修改。
...
- [ ] 第一批迁移的同一个 PR 内修改 `AGENTS.md`：写明 iOS 规则由 Swift 实现、TS 作为共有行为的规格与差分预言机、iOS 专属规则只在 Swift 中实现；更新 002 中“关于 JS 桥”的结论。不允许出现代码已迁移而文档仍禁止迁移的中间状态。
...
- [ ] **R1 排班与快照**：`snapshot`、`expandScheduleRange`、`widgetShifts`、`watchProjection`、`validateBreak`。这是 018 P8 的前置条件。
